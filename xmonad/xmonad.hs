...
myManageHook = fullscreenManageHook <+> manageDocks <+> composeAll
    ...
    , className =? "gazebo"           --> doShift ( myWorkspaces !! 4 )
    , className =? "Gui_node"         --> doFloat
    , className =? "rviz2"   --> doShift ( myWorkspaces !! 3 )
    , (className =? "Matplotlib" <&&> title =? "Figure 1") -->doShift ( myWorkspaces !! 8 )
    , (className =? "rviz" <&&> title =? "tare_planner_ground.rviz - RViz") -->doShift ( myWorkspaces !! 3 )
    , (className =? "rviz" <&&> title =? "vehicle_simulator.rviz - RViz") -->doShift ( myWorkspaces !! 8 )
                                 ]
...
myStartupHook = do
  ...
  spawn "exec ~/bin/set_display.sh"
  -- spawnOnce "feh  --randomize --bg-scale --no-xinerama ~/wallpapers/*"
...
defaults = def {
      ...
      -- hooks, layouts
        ...
        layoutHook = gaps [(L,4), (R,4), (U,40), (D,4)] $ spacingRaw True (Border 4 4 4 4) True (Border 4 4 4 4) True $ smartBorders $ myLayout,
